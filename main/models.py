...
class Establishment(models.Model):
    ...
    departments = models.ManyToManyField(
        Department, blank=False, related_name="establishments")
...
class UsefulLink(models.Model):
    ...
    department = models.ForeignKey(
        Department, on_delete=models.CASCADE, default=None, blank=False, null=True)
...
class Year(models.Model):
    ...
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, default=None, blank=True, null=True)
    first_semester = models.ForeignKey(
        Semester, related_name="first_semester", on_delete=models.CASCADE, default=None, blank=False, null=True)
    second_semester = models.ForeignKey(
        Semester, related_name="second_semester", on_delete=models.CASCADE, default=None, blank=False, null=True)
...
class Group(models.Model):
    ...
    year = models.ForeignKey(
        Year, on_delete=models.CASCADE, default=None, blank=False, null=True)
...
class Evaluation(models.Model):
    ...
    professor = models.ForeignKey(
        Professor, on_delete=models.CASCADE, default=None, blank=False, null=True)
    semester = models.ForeignKey(
        Semester, on_delete=models.CASCADE, default=None, blank=False, null=True)
...
def loadInformatique():
    ...
    establishment = Establishment(
        name="IUT Claude Bernard Lyon 1 - Bourg en bresse")
    establishment.save()
    establishment.departments.add(department)

    ue1_1 = UE(
        number=1, description="Développer des applications informatiques simples")
    ue1_1.save()
    ue1_2 = UE(
        number=1, description="Partir des exigences et aller jusqu'à une application complète")
    ue1_2.save()
    ue1_3 = UE(
        number=1, description="Adapter des applications sur un ensemble de supports (embarqué, web, mobile, IoT…)")
    ue1_3.save()

    ue2_1 = UE(number=2, description="Appréhender et construire des algorithmes")
    ue2_1.save()
    ue2_2 = UE(
        number=2, description="Sélectionner les algorithmes adéquats pour répondre à un problème donné")
    ue2_2.save()
    ...
    ue3_1.save()
    ue3_2 = UE(
        number=3, description="Déployer des services dans une architecture réseau")
    ue3_2.save()
    ...
    ue5_1 = UE(
        number=5, description="Identifier les besoins métiers des clients et des utilisateurs")
    ue5_1.save()
    ...
    ue5_2.save()
    ue5_3 = UE(
        number=5, description="Participer à la conception et à la mise en oeuvre d'un projet système d'information")
    ue5_3.save()

    ue6_1 = UE(
        number=6, description="Identifier ses aptitudes pour travailler dans une équipe")
    ue6_1.save()
    ue6_2 = UE(
        number=6, description="Situer son rôle et ses missions au sein d'une équipe informatique")
    ue6_2.save()
    ...
    year1 = Year(name="Première année", first_semester=semester1,
                 second_semester=semester2)
    year1.save()
    year2 = Year(name="Première année", first_semester=semester3,
                 second_semester=semester4)
    year2.save()
    year3 = Year(name="Première année", first_semester=semester5,
                 second_semester=semester6)
    year3.save()

    groupS1 = Group(name="S1", department=department, parent=None, year=year1)
    groupS1.save()
    groupS1G1 = Group(name="S1G1", department=department,
                      parent=groupS1, year=year1)
    groupS1G1.save()
    groupS1G1_1 = Group(name="S1G1.1", department=department,
                        parent=groupS1, year=year1)
    groupS1G1_1.save()

    noa = Student(id=User.objects.all().filter(
        username="noa_cavalcante").first())
    noa.save()
    noa.groups.add(groupS1, groupS1G1, groupS1G1_1)

    lilian = Student(id=User.objects.all().filter(
        username="lilian_ouraha").first())
    lilian.save()
    lilian.groups.add(groupS1, groupS1G1, groupS1G1_1)

    corto = Student(id=User.objects.all().filter(
        username="corto_bouviolle").first())
    corto.save()
    corto.groups.add(groupS1, groupS1G1, groupS1G1_1)

    peytavie = Professor(id=User.objects.all().filter(
        username="adrien_peytavie").first())
    peytavie.save()
